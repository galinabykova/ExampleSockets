...
class ClientServerTest {
    ...
    void simpleFalseTest(SleepingBoss boss, SleepingWorker worker) {
        try {
            ...
            serverThread.start();
            SleepingResult result = worker.Work(new SleepingTask(1,
                    new int[] {0}));
        ...
        try {
            ...
            for (int i = 0; i < nTasks; ++i) {
                SleepingResult result =
                        worker.Work(new SleepingTask(100,
                                new int[] {0, 1, 2, 3, 4, 5, 6, 7}));
                Assertions.assertTrue(result.result);
            }
            boss.close();
    ...
    void notServerTest(SleepingWorker worker) {
        Assertions.assertThrows(IOException.class,
                    () -> worker.Work(new SleepingTask(1, new int[] {0})));
    }

    private static Stream<Arguments> provideArguments() throws IOException {
        return Stream.of(
                Arguments.of(new TcpSleepingBoss(PORT),
                        new TcpSleepingWorker("localhost", PORT)),
                Arguments.of(new UdpSleepingBoss(PORT),
                        new UdpSleepingWorker("localhost", PORT))
        );
    }
